...
# Distinct default from the direct variant's corebackup.prom, so the two
# scripts can coexist on one host without clobbering each other's metrics.
METRICS_FILE="${METRICS_FILE:-/var/lib/node_exporter/textfile/corebackup-gw.prom}"
...
# variant label distinguishes these series from the direct script's; the
# write-then-rename keeps the collector from scraping a half-written file.
...
corebackup_last_success_timestamp_seconds{variant="gw"} $(date +%s)
# HELP corebackup_last_size_bytes Size of last core backup payload.
# TYPE corebackup_last_size_bytes gauge
corebackup_last_size_bytes{variant="gw"} ${SIZE_BYTES}
EOF
