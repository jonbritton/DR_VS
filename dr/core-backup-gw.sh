...
GW_MOUNT="${GW_MOUNT:?set GW_MOUNT}"                    # NFS mount of the File Gateway share, e.g. /mnt/dr-gw
FILE_SHARE_ARN="${FILE_SHARE_ARN:?set FILE_SHARE_ARN}" # used to request an upload-complete notification
...
# The mount must be the live gateway share, not an empty dir left behind when it
# is unmounted (writing there would silently bypass S3)
